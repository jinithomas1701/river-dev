...
import VoiceDock from './VoiceDock/VoiceDock';
import CommitmentDock from './CommitmentDock/CommitmentDock';
import CUDDock from './CUDDock/CUDDock';
import ActivityItem from './ActivityItem/ActivityItem';
import VoiceItem from './VoiceItem/VoiceItem';
import ActivityListItem from './ActivityListItem/ActivityListItem';
import AppConfig from "../../Util/Constants/AppConfig";
import AssignedDetailDock from './ActivityTile/AssignedDetailDock/AssignedDetailDock';
...
class CommonDashboard extends Component {
    state = {
        ...
        assignedActivityDetail: {masterActivity: undefined, activity: undefined},
        activityDiscussions: [],
...
componentDidMount() {
    ...
    const loggedUser = Util.getLoggedInUserDetails();

    if(this.activeRole == 'ROLE_CLUB_PRESIDENT') {
        this.checkClubSetupStatus();
    } else if(this.activeRole == 'ROLE_RIVER_COUNCIL') {
        const canViewVoice = Util.hasPrivilage(PRIVILEGE_VIEW_VOICE);
        this.councilDashInit();
        if(canViewVoice && (loggedUser.voiceDepartments.length || loggedUser.voiceEscalationPanels.length)){
            this.sidebarBtns.push({
                title: "Voices",
                value: 1,
                icon: 'volume_down',
...
componentDidUpdate(prevProps, prevState) {
    if(this.activeRole !==  Util.getActiveRole().value){
...
render() {
    ...
    .map((item, index) => {
        return <ActivityListItem
                   ...
                   onChangeSelectedTile={this.onChangeSelectedTile.bind(this)}
                   handleAssignedDockVisibility={this.handleAssignedDockVisibility.bind(this)}
                   isAssignedDockVisible={this.state.isAssignedDockVisible}
                   />
        // return  <ActivityItem
        //             key={index}
        //             activity={item}
        ...
        //             onAssignUsers={this.assignSuccess.bind(this)}
    ...
    .map((item, index) => {
        return  <VoiceItem
                    ...
                    voice={item}
                    index={index}
                    onVoiceClick={this.onVoiceClick.bind(this)}
    ...
    .map((item, index) => {
        return  <div key={index}  className="item-tile">
            <div className="title" onClick={this.onCommitmentClick.bind(this, item, index)}>{item.title}</div>
            <div className="status">{item.currentStatusName}</div>
        </div>
    })
    : false;

    const cudsList = (this.props.commonDash.cudsList)
    ? this
    .props
    .commonDash
    .cudsList
    .map((item, index) => {
        return  <div key={index}  className="item-tile">
            <div className="title" onClick={this.onCudClick.bind(this, item, index)}>{item.title}</div>
            <div className="status">{item.currentStatusName}</div>
        </div>
    })
    : false;
    ...
    const dashboardTabs = this.sidebarBtns.map((item, index) => {
        const statusClass = this.state.summaryTableTabValue == item.value ? "active" : "";
        return <div
                   className={`last-weeks-sidebar-btn ${statusClass}`}
                   key={item.value}
                   onClick={this.handleSummaryTableTabChange.bind(this, item.value)} >
    ...
    return (
        <Root role="admin">
            <MainContainer>
                    ...
                    this.state.setupDone ? 
                        <div className="common-dashboard-container">
                            <div className="dashboard-datas">
                                ...
                                {
                                    (this.activeRole == 'ROLE_CLUB_PRESIDENT') && (
                                        ...
                                        <div className="last-weeks-table-container activities">
                                            ...
                                            <div className="last-weeks-table-content">
                                                {/* <div className="filter-options">
                                                    ...
                                                    {
                                                        this.state.activityStatuses.length > 0 &&
                                                            <div className="filter-item">
                                                    ...
                                                    {
                                                        this.activeRole == 'ROLE_RIVER_COUNCIL' &&
                                                            <div className="filter-item">
                                                ...
                                                <div className="activities-container">
                                                    ...
                                                    <div className="label-container">
                                                        ...
                                                        <div className="label">
                                                            ...
                                                            {this.activeRole == 'ROLE_RIVER_COUNCIL' ?
                                                                <div className="text">Pending for approval</div>:
                ...
                <Dock size={0.5} zIndex={200} position='right' isVisible={this.state.isAssignedDockVisible} dimMode="opaque" defaultSize={.4}>
                    {/*<AssignedDetailDock 
                        ...
                        activity={this.state.assignedActivityDetail}
                    ...
                    <AssignedDetailDock
                        ...
                        disscussionSubmitProgress={this.state.disscussionSubmitProgress}
                        discussions={this.state.activityDiscussions}
                        masterActivity={this.state.assignedActivityDetail.masterActivity} 
                        activity={this.state.assignedActivityDetail.activity}
                        onCloseDock={this.closeAssignedActivityDetailDock.bind(this)}
                        />
                </Dock>
            </MainContainer>
        </Root>
...
councilDashInit() {
    ...
    //         title: "CUDS",
    //         value: 2,
    //         count: this.props.commonDash.cudsList.length || 0
    //     }
    // );
...
handleAssignedDockVisibility(visibility, masterActivity, activityId){
    if(visibility){
        this.setState({selectedTile: activityId})
        this.loadActivityDockDetail(masterActivity, activityId);
    }
    else{
        this.setState({
            isAssignedDockVisible: visibility,
            selectedTile: false
        });
    }
}

loadActivityDockDetail(masterActivity, activityId){
    CommonDashboardService.getActivityDockDetail(masterActivity.id, activityId)
        .then(data => {
        /*
        this.loadDiscussions(data.discussionId);
        this.setState({assignedActivityDetail: data});*/
        //this.props.onChangeSelectedTile(activityId);
        this.loadDiscussions(data.discussionId);
        const assignedActivityDetail = {
            masterActivity: masterActivity,
            activity: data
        };
        this.setState({
            isAssignedDockVisible: true,
            assignedActivityDetail: assignedActivityDetail
        });
    })
        .catch(error => {
        console.log(error);
...
loadDiscussions(discussionId) {
    CommonDashboardService.loadActivityComments(discussionId)
        .then(data => {
...
onSubmitDiscussionMessage(message) {
    if (message.trim()) {
        const messageRequest = {
            value: message,
            commentId: this.currentDiscussionId
        };
        ...
        CommonDashboardService.postDiscussionMessage(messageRequest)
            ...
            this.loadDiscussions(this.currentDiscussionId);
        })
            .catch(error => {
            this.setState({ disscussionSubmitProgress: false });
...
closeAssignedActivityDetailDock(){
    this.setState({
        isAssignedDockVisible: false,
        assignedActivityDetail: {masterActivity: undefined, activity: undefined},
        selectedTile: false
...
createNewCommitment() {
    ...
    if (commitmentTitle && commitmentDescription && commitmentStatus) {
        ...
        CommonDashboardService.createCommitment(request)
            .then(data => {
...
getVoiceTypes() {
    CommonDashboardService.getVoiceTypes()
        .then((data) => {
...
getWizardActivityList(activity){
    CommonDashboardService.getWizardActivityMasters(activity)
        .then((data) => {
        this.setState({
            ...this.state,
            isVisible: true,
...
getFocusAreas() {
    CommonDashboardService.getFocusAreasList()
        .then((data) => {
        const list = data || [];
...
getCouncilActivities(pageNo = 0) {
    ...
    CommonDashboardService.getCouncilActivities(pageNo, urlQuery)
        .then((data) => {
        if(pageNo === 0) {
            this
                .props
...
getActivities(pageNo = 0, searchTerm = '', expand = false) {
    ...
    CommonDashboardService.getActivities(pageNo, urlQuery, searchTerm)
        .then((data) => {
...
getRecentMeeting() {
    CommonDashboardService.getRecentMeeting()
        .then((data) => {
...
getCommitmentsStatuses() {
    CommonDashboardService.getCommitmentsStatuses()
        .then(statusList => {
...
loadClubFilters() {
    CommonDashboardService.getClubsList()
        .then((data) => {
        const list = data || [];
...
startStopMeeting(meeting) {
    ...
    CommonDashboardService.startStopMeetingTask(meeting.meetingId, request)
        .then((data) => {
