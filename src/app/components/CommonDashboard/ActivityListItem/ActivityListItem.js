...
export default class ActivityListItem extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState){
        //@DESC: This is for a rare case bug when customised points is changed in ActivityTile when more-info dock is opened.
        //The below code prevents rerender and hence change of value through props propagation.
        return (!this.props.isAssignedDockVisible && !nextProps.isAssignedDockVisible);
    }
...
getActivityHead(activity) {
    ...
    return <div className="activity-head" id={this.props.isExpand ? this.props.isExpand : ''}>
        ...
        <div className="bottom-section">
            ...
            {currentRole == 'ROLE_CLUB_PRESIDENT' ?
            ...
            {/*<div className="point-section" title="Member point: The maximum a member can score">
                <Icon>person</Icon>
                <div className="point">{activity.memberPoint || 0}</div>
            </div>
            <div className={currentRole == 'ROLE_CLUB_PRESIDENT' ? "point-section" : "point-section no-border-right"} title="Club point">
                <Icon>store</Icon>
                <div className="point">{activity.clubPoint || 0}</div>
            </div>*/}
            {(currentRole == 'ROLE_CLUB_PRESIDENT' && !AppConfig.isActivityAssignmentDisabled) && <div className="action-section" title={this.state.isAssigningActivity ? 'Close' : 'Assign activity'} onClick={this.onAssignNewActivityClick.bind(this, activity)}>
...
getActivityBody(masterActivity) {
    ...
    if (this.state.assignedActivityList && this.state.assignedActivityList.length > 0) {
        assignedActivityElements = this.state.assignedActivityList.map((activity, index) => {
            const activityKey = `${activity.id}-${Math.random()}`;
            return <ActivityTile
                       key={activityKey}
                       className={"activity-tile"}
                       onActionCompleted={this.onActionCompleted.bind(this)}
                       masterActivity={masterActivity}
                       activity={activity}
                       updateCount={this.updateCount.bind(this)}
                       selectedTile={this.props.selectedTile}
                       handleAssignedDockVisibility={this.props.handleAssignedDockVisibility}
                       />
...
getActivityFullDetail(){
    ...
    CommonDashboardService.mainActivitiesFullDetails(roleVariable,masterActivityId,"full")
        .then((data) => {
...
onActionCompleted(action) {
    ...
    if(action === 'delete'){
        ...
        CURRENT_PAGE_NO = 0;
        this.activityExpandClick(masterActivity, 'partial', searchTerm, selectedStatusFilter, CURRENT_PAGE_NO, pageSize);
    }
    else{
        this.activityExpandClick(masterActivity);
    }
...
activityExpandClick(masterActivity, context = 'partial', searchString = '', status = '', pageNo = 0, pageSize = PAGE_SIZE) {
    ...
    CommonDashboardService.getActivityDetail(masterActivity.id, context, (searchString || this.state.searchTerm), statusFilter, pageNo, pageSize)
        .then((data) => {
        if (data && data.activityInfo) {
            ...
            this.props.onChangeSelectedTile(false);
        }
