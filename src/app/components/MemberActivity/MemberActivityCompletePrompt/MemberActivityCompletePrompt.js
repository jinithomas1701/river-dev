...
// css
import "./MemberActivityCompletePrompt.scss";

const RATING_TYPE_STAR = "S";
const RATING_TYPE_CATEGORY = "C";
const ACTIVITY_STATUS_ACTIVE = "A";

class MemberActivityCompletePrompt extends Component{
    constructor(props){
        super(props);
        this.state = {
            comment: ""
    ...
    getMainTemplate(activity){
        ...
        const userId = Util.getLoggedInUserDetails().userId;
        const currentUser = activity ? activity.pointMatrix.assignees.find(item => {
            return (userId === item.userId);
        }) : null;
        const attachmentList = currentUser? currentUser.attachments : [];

        return (
            <Dialog
                open={this.props.open}
                maxWidth="md"
                className="member-activitycomplete-wrapper"
                >
                <DialogTitle className="header">Complete Activity</DialogTitle>
                ...
                <DialogActions className="submit-wrapper">
                    ...
                    <Button className="btn-primary" color="primary" onClick={this.handleComplete}>Complete</Button>
                </DialogActions>
            </Dialog>
        );
    }
    ...
    getAssigneesList(assigneesList){
        const assignees = assigneesList.map(assignee => ({
            id: assignee.id,
            title: assignee.name,
            subText: `${assignee.calculatedPoints}`,
...
MemberActivityCompletePrompt.propTypes = {
...
export default MemberActivityCompletePrompt;