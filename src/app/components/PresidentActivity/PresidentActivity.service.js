...
export default class PresidentActivityService {
    ...
    static approveActivity(activityId, completeObj, role = ROLE_PRESIDENT) {
    ...
    static forceStopActivity(activityId) {
        let url = ApiUrlConstant.getApiUrl("forceStop");
        url = Util.beautifyUrl(url, [activityId]);
    ...
    static createActivity(role, masterId, activityObj) {
        let url = ApiUrlConstant.getApiUrl("createAssignedActivity");
        url += `?role=${role}&master=${masterId}`;

        return Api.doPost(url, activityObj)
            .then((resp) => {
                if (resp && resp.status_code == "200") {
                    return resp.payload;
                }
    ...
    static resubmitActivity(activityId, role) {
    ...
    static getClubMembers(kpiId) {
        let url = ApiUrlConstant.getApiUrl("getAssignedActivityAssignees");
        url = Util.beautifyUrl(url, [kpiId]);
