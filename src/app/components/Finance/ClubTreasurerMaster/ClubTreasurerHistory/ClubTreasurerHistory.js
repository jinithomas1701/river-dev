...
import {
    setClubTreasurerHistorySearch, setClubTreasurerHistoryType, setClubTreasurerHistoryStatus,
    setClubTreasurerHistoryGroupBy, setClubTreasurerHistoryList, setClubTreasurerHistoryDockTransactionDetails,
    setClubTreasurerHistoryDockDiscussionDetails, clearClubTreasurerHistoryDockDetails
} from './ClubTreasurerHistory.action';
...
import './ClubTreasurerHistory.scss';
...
const mapStateToProps = (state) => {
    return {
        ClubTreasurerHistoryStore: state.ClubTreasurerHistoryReducer
    }
};
...
const mapDispatchToProps = (dispatch) => {
    return {
        setFilterSearchText: (filterSearchText) => {
            dispatch(setClubTreasurerHistorySearch(filterSearchText));
        },
        setFilterType: (filterType) => {
            dispatch(setClubTreasurerHistoryType(filterType));
        },
        setFilterStatus: (filterStatus) => {
            dispatch(setClubTreasurerHistoryStatus(filterStatus));
        },
        setFilterGroupBy: (filterGroupBy) => {
            dispatch(setClubTreasurerHistoryGroupBy(filterGroupBy));
        },
        setHistoryList: (historyList) => {
            dispatch(setClubTreasurerHistoryList(historyList));
        },
        setDockTransactionDetails: (transaction) => {
            dispatch(setClubTreasurerHistoryDockTransactionDetails(transaction));
        },
        setDockDiscussionDetails: (discussion) => {
            dispatch(setClubTreasurerHistoryDockDiscussionDetails(discussion));
        },
        clearDockTransactionDetails: () => {
            dispatch(clearClubTreasurerHistoryDockDetails());
        }
    }
};

class ClubTreasurerHistory extends Component {
    ...
    render() {
        return (
            <div className="club-treasurer-history-wrapper">
                <div className="create-button-wrapper">
                    {
                        this.props.privileges.canCreateTransaction &&
                        <Button
                            className="btn-primary"
                            onClick={this.handleCreateTransactionDialog}
                        >
                            Create Transaction
                        </Button>
                    }
                ...
                <CreateTransactionDialog
                    ...
                    closeDialogHandler={this.handleCloseDialog}
                ...
                <Filters
                    ...
                    selectedType={this.props.ClubTreasurerHistoryStore.filterType}
                    selectedStatus={this.props.ClubTreasurerHistoryStore.filterStatus}
                    selectedGroupBy={this.props.ClubTreasurerHistoryStore.filterGroupBy}
                ...
                <StackedList
                    listData={this.props.ClubTreasurerHistoryStore.historyList}
                ...
                {
                    this.props.ClubTreasurerHistoryStore.historyList.length > 8 &&
                    <Pagination
                        page={this.state.pageNo}
                        pageSize={this.state.pageSize}
                        totalPages={undefined}
                ...
                <a style={{ "display": "none" }} id="download-anchor" ></a>
                {
                    ...
                    >
                        <ClubTreasurerListItemDockDetails
                            ...
                            transaction={this.props.ClubTreasurerHistoryStore.dockTransactionDetails}
                            discussion={this.props.ClubTreasurerHistoryStore.dockDiscussionDetails}
    ...
    loadHistoryList(
        type = this.props.ClubTreasurerHistoryStore.filterType,
        status = this.props.ClubTreasurerHistoryStore.filterStatus,
        groupBy = this.props.ClubTreasurerHistoryStore.filterGroupBy,
        searchText = this.props.ClubTreasurerHistoryStore.filterSearchText,
        page = this.state.pageNo,
        count = this.state.pageSize,
    ) {

        FinanceService.getClubTreasurerHistoryList(type, status, groupBy, searchText, page, count)
            .then(historyList => {
    ...
    handlePageForward = (pageData) => {
        ...
        let type = this.props.ClubTreasurerHistoryStore.filterType;
        let status = this.props.ClubTreasurerHistoryStore.filterStatus;
        let groupBy = this.props.ClubTreasurerHistoryStore.filterGroupBy;
        let searchText = this.props.ClubTreasurerHistoryStore.filterSearchText;
        let count = this.state.pageSize;
    ...
    comparator = (prevItem, currItem) => {
        const groupBy = this.props.ClubTreasurerHistoryStore.filterGroupBy;
...
export default connect(mapStateToProps, mapDispatchToProps)(ClubTreasurerHistory);