...
import './AdminCustomizePointsDialog.scss';
import { Util } from "../../../../../../Util/util";
import { Avatar, Icon } from "material-ui";
import { Toast, riverToast } from '../../../../../Common/Toast/Toast';

export default class AdminCustomizePointsDialog extends React.Component {
        ...
        assigneesList = assignees.map((assignee, index) => {
            return <div className="list-item" key={index}>
                ...
                <div className="tail-container">
                    <ClubInput className="point-value" placeholder="Point" onChange={(e) => this.handlePointValue('assignees', assignee, "id",e.target.value)} value={assignee.calculatedPoints} disabled={true} />
    ...
    if (clubs && clubs.length > 0) {
        clubList = clubs.map((club, index) => {
            return <div className="list-item" key={index}>
                ...
                <div className="tail-container">
                    <ClubInput className="point-value" placeholder="Point" onChange={(e) => this.handlePointValue('clubs', club, "id", e.target.value)} value={club.calculatedPoints} disabled={true} />
    ...
    return ( 
        <Dialog open={this.props.open} onRequestClose={this.handleRequestClose.bind(this)} className="customize-points-dialog-container">
            ...
            <DialogContent>
                ...
                {(activity.status == 'C' && adminPrivilige == 'VIEW_ADMIN_DASHBOARD') || (activity.status == 'PA' && adminPrivilige == 'ROLE_RIVER_COUNCIL') ?
                    <div className="justification-wrapper">
...
viewActivityPointHistory() {
    ...
    AdminActivityTileService.viewActivityPointHistory(masterActivity.id, activity.id)
        .then((data) => {
